...
describe("Set Delivery Time PO ", async () => {
  ...
  it("Check Tab Approve", async () => {
    ...
    await expect(PopupDeliveryStatus.TabApprove).toBeDisplayed();
});

    it('Check Click Btn Set Delevery',async () => {
        await PopupDeliveryStatus.TabApprove.click ();
        await PopupDeliveryStatus.BtnSetDelivery.click ();
        const isAlertPresent = await browser.isAlertOpen();
        expect(isAlertPresent).to.be.true;
        const alertText = await browser.getAlertText();
        expect(alertText).to.equal('Pls choice your purchase order record to set delivery.');
        browser.acceptAlert();
